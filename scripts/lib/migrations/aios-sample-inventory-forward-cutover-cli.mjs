...
export function parseSampleInventoryForwardCutoverArgs(args) {
  const [command, ...options] = args;
  if (!command || command === '--help' || command === '-h') return { help: true };
  if (!['verify', 'apply'].includes(command)) {
    throw new Error(`Unsupported sample inventory forward cutover command: ${command}.`);
  }
  const parsed = {
    command,
    confirmed: false,
  ...
  for (let index = 0; index < options.length; index += 1) {
    ...
    if (!matched) throw new Error(`Unknown sample inventory forward cutover option: ${argument}.`);
    const value = argument.slice(matched[0].length + 1);
    if (!value) throw new Error(`${matched[0]} requires a value.`);
    parsed[matched[1]] = value;
  }
  if (!parsed.outputPath) throw new Error('--output is required for sample inventory forward cutover.');
...
export function resolveSampleInventoryForwardCutoverAuthorization({ command, confirmed, env }) {
  ...
  if (command === 'verify') {
    if (env.AIOS_SAMPLE_INVENTORY_CUTOVER_ALLOW_LIVE_READONLY !== '1') {
      throw new Error('Verify requires AIOS_SAMPLE_INVENTORY_CUTOVER_ALLOW_LIVE_READONLY=1.');
    }
  } else if (confirmed !== true
    || env.AIOS_SAMPLE_INVENTORY_CUTOVER_WRITE_ACK !== 'apply-backend-014-v1') {
    throw new Error(
...
export function formatSampleInventoryForwardCutoverResult(result, artifact) {
  ...
  return [
    ...
    `effects=${topology.effectsPresent}/${topology.expectedEffects ?? 30} ledger=${topology.ledgerExists ? 'present' : 'missing'}`,
