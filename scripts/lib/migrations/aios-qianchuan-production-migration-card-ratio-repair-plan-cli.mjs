...
export function parseQianchuanCardRatioRepairPlanArgs(args) {
  const options = {
    help: false,
    json: false,
    outputPath: null,
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === '--help' || argument === '-h') options.help = true;
      ...
      if (!matched) throw new Error(`Unknown qianchuan card-ratio repair-plan option: ${argument}.`);
  ...
  if (!options.help) {
    for (const [option, field] of VALUE_OPTIONS) {
      if (option !== '--output' && !options[field]) {
        throw new Error(`${option} is required for the offline card-ratio repair plan.`);
      }
    }
  }
  return options;
...
export function formatQianchuanCardRatioRepairPlan(plan, artifact = null) {
  const lines = [
    '[qianchuan-card-ratio-repair-plan]',
    `mode=${plan.mode} ready=${plan.decision.repairPlanReady} strategy=${plan.decision.recommendedStrategy}`,
    `canary_date=${plan.execution.canary.date} canary_rows=${plan.execution.canary.mismatchRows} batch_size=${plan.execution.batching.maxRowsPerTransaction} expected_batches=${plan.execution.batching.expectedBatches}`,
