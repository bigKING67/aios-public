...
export function parseSampleInventoryPublicCutoverArgs(args) {
  const [command, ...options] = args;
  if (!command || command === '--help' || command === '-h') return { help: true };
  if (!['verify', 'apply'].includes(command)) {
    throw new Error(`Unsupported sample inventory public cutover command: ${command}.`);
  }
  const parsed = {
    command,
    confirmed: false,
  ...
  for (let index = 0; index < options.length; index += 1) {
    ...
    if (argument === '--confirm-public-cutover') {
      if (command !== 'apply') throw new Error('--confirm-public-cutover is valid only for apply.');
    ...
    if (!matched) throw new Error(`Unknown sample inventory public cutover option: ${argument}.`);
    const value = argument.slice(matched[0].length + 1);
    if (!value) throw new Error(`${matched[0]} requires a value.`);
    parsed[matched[1]] = value;
  }
  if (!parsed.outputPath) throw new Error('--output is required for sample inventory public cutover.');
...
export function resolveSampleInventoryPublicCutoverAuthorization({ command, confirmed, env }) {
  ...
  if (command === 'verify') {
    if (env.AIOS_SAMPLE_INVENTORY_PUBLIC_CUTOVER_ALLOW_LIVE_READONLY !== '1') {
      throw new Error('Verify requires AIOS_SAMPLE_INVENTORY_PUBLIC_CUTOVER_ALLOW_LIVE_READONLY=1.');
    }
  } else if (confirmed !== true
    || env.AIOS_SAMPLE_INVENTORY_PUBLIC_CUTOVER_WRITE_ACK !== 'apply-backend-015-v1') {
    throw new Error(
      'Apply requires --confirm-public-cutover and '
  ...
  if (expectedDatabaseIdentitySha256 !== null
    ...
    throw new Error('AIOS_SAMPLE_INVENTORY_EXPECTED_DATABASE_IDENTITY_SHA256 must be 64 lowercase hex characters.');
  }
  if (command === 'apply' && expectedDatabaseIdentitySha256 === null) {
    throw new Error('Apply requires AIOS_SAMPLE_INVENTORY_EXPECTED_DATABASE_IDENTITY_SHA256 from the final verify artifact.');
  }
  return { expectedDatabase, expectedDatabaseIdentitySha256, expectedGitSha };
}

export function formatSampleInventoryPublicCutoverResult(result, artifact) {
  const topology = result.topology?.after ?? result.topology;
  return [
    '[sample-inventory-public-cutover]',
    ...
    `backend_014=${topology.baseEffectsPresent}/${topology.expectedBaseEffects} backend_015=${topology.publicEffectsPresent}/${topology.expectedPublicEffects} ledger=${topology.ledgerExists ? 'present' : 'missing'}`,
