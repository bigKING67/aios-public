const VALUE_OPTIONS = new Map([
  ['--catalog-probe', 'catalogProbePath'],
  ['--catalog-probe-sha256', 'catalogProbeSha256'],
  ['--lineage', 'lineagePath'],
...
export function parseQianchuanProductionMigrationP1cReviewDecisionArgs(args) {
  const options = {
    catalogProbePath: null,
    catalogProbeSha256: null,
    help: false,
    json: false,
    lineagePath: null,
    lineageSha256: null,
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === '--help' || argument === '-h') options.help = true;
      ...
      if (!matched) throw new Error(`Unknown qianchuan migration P1C review decision option: ${argument}.`);
  ...
  if (!options.help) {
    for (const [option, field] of VALUE_OPTIONS) {
      if (!options[field]) throw new Error(`${option} is required for the offline P1C owner review decision.`);
    }
  }
  return options;
}

export function formatQianchuanProductionMigrationP1cReviewDecisions(result, artifact) {
  return [
    '[qianchuan-production-migration-p1c-review-decisions]',
    `mode=${result.mode} decisions=${result.summary.decisions} prior=${result.summary.priorDecisions} new_p1c=${result.summary.newDecisions} reviewer=${result.authorization.reviewer}`,
    ...
    `catalog_probe=${result.sourceArtifacts.catalogProbe.path} sha256=${result.sourceArtifacts.catalogProbe.sha256}`,
