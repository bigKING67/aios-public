...
#[derive(Clone, Copy)]
pub(super) struct AttributionWindow {
...
pub(super) async fn resolve_attribution_window(
    ...
    week_end: NaiveDate,
) -> AppResult<Option<AttributionWindow>> {
    ...
    let max_source_date = match sqlx::query(sql::MAX_SOURCE_DATE).fetch_one(pool).await {
        ...
        Err(error) if is_undefined_table(&error) => {
            warn!(
                ?error,
                "weekly douyin card attribution skipped, source table missing"
            );
            return Ok(None);
        }
        Err(error) => {
            error!(?error, "query douyin card max stat date failed");
