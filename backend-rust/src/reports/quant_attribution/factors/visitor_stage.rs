...
pub(super) fn build_visitor_stage_factor_rows(totals: &ChannelMetricTotals) -> Vec<FactorRow> {
    let curr_click_to_cart_rate =
        safe_ratio(totals.curr_cart_sum as f64, totals.curr_visitor_sum as f64).unwrap_or(0.0);
    let prev_click_to_cart_rate =
        safe_ratio(totals.prev_cart_sum as f64, totals.prev_visitor_sum as f64).unwrap_or(0.0);
    ...
    vec![
        FactorRow {
            factor_key: "visitor",
            factor_label: "访客",
            curr_value: totals.curr_visitor_sum as f64,
            prev_value: totals.prev_visitor_sum as f64,
            ln_contribution: ln_ratio_with_smoothing(
                totals.curr_visitor_sum as f64,
                totals.prev_visitor_sum as f64,
                1.0,
            ),
        },
        FactorRow {
