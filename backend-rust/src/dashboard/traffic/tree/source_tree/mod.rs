...
use super::{node::build_traffic_node_value, row_ops::sort_traffic_rows};
...
pub(in crate::dashboard) fn build_traffic_source_tree(
    ...
    sort_traffic_rows(level1_rows.as_mut_slice());
...
fn sort_grouped_rows(grouped_rows: &mut HashMap<String, Vec<DashboardTrafficMetricRow>>) {
    for rows in grouped_rows.values_mut() {
        sort_traffic_rows(rows.as_mut_slice());
    }
}

fn build_known_source_roots(
    level1_rows: Vec<DashboardTrafficMetricRow>,
    level2_by_parent: &mut HashMap<String, Vec<DashboardTrafficMetricRow>>,
    ...
    level1_rows
        ...
        .map(|level1_row| {
            ...
            build_traffic_node_value(&level1_row, level2_nodes)
        })
        .collect()
}
