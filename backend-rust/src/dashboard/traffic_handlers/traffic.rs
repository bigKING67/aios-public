...
use super::super::errors::normalize_traffic_error_message;
...
use super::super::traffic::{
    build_traffic_source_tree, get_traffic_as_of_date_sql, get_traffic_metrics_sql,
    traffic_metric_row_from_pg_row, DashboardTrafficMetricRow,
};
use super::common::{as_of_date_from_rows, validate_traffic_query, ValidatedTrafficQuery};

pub(in crate::dashboard) async fn get_traffic(
    ...
    let mut connection = match state.pool.acquire().await {
        ...
        Err(error) => {
            ...
            let message = normalize_traffic_error_message(raw_error.as_str());
            error!(target: "dashboard-traffic", raw_error = %raw_error, "db acquire failed");
    ...
    let phase_started_at = Instant::now();
    let metrics_payload_rows = match sqlx::query(get_traffic_metrics_sql())
        ...
        .await
    {
        ...
        Err(error) => {
            let raw_error = error.to_string();
            let message = normalize_traffic_error_message(raw_error.as_str());
            error!(target: "dashboard-traffic", raw_error = %raw_error, "query failed");
            return json_message_response(StatusCode::SERVICE_UNAVAILABLE, message.as_str());
        }
    };
    ...
    let mut metrics_rows = Vec::<DashboardTrafficMetricRow>::new();
    for row in metrics_payload_rows {
        ...
        let metric_row = match traffic_metric_row_from_pg_row(&row) {
            Ok(value) => value,
            Err(error) => {
                let raw_error = error.to_string();
                let message = normalize_traffic_error_message(raw_error.as_str());
    ...
    let as_of_date = if let Some(value) = metrics_as_of_date {
        ...
        let phase_started_at = Instant::now();
        let as_of_rows = match sqlx::query(get_traffic_as_of_date_sql())
            .bind(start_date.as_str())
            .bind(end_date.as_str())
    ...
    let phase_started_at = Instant::now();
    let tree = build_traffic_source_tree(metrics_rows.as_slice());
