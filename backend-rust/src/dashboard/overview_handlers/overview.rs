...
use super::super::{
    errors::normalize_overview_error_message,
    overview::{build_overview_query_sql, OverviewSqlOptions},
    params::OverviewQueryParams,
    query_runtime::{apply_overview_nowcast_prediction, run_dashboard_query_json},
...
pub(in crate::dashboard) async fn get_overview(
    State(state): State<Arc<AppState>>,
    Query(query): Query<OverviewQueryParams>,
) -> Response {
    ...
    let include_platform_share =
        parse_include_platform_share(query.include_platform_share.as_deref());
    ...
    let sql = build_overview_query_sql(OverviewSqlOptions {
    ...
    let mut payload = match run_dashboard_query_json(&state.pool, sql.as_str()).await {
        Ok(value) => value,
        Err(raw_error) => {
            let message = normalize_overview_error_message(raw_error.as_str());
            error!(target: "dashboard-overview", raw_error = %raw_error, "query failed");
            return json_message_response(StatusCode::SERVICE_UNAVAILABLE, message.as_str());
        }
    };

    if let Err(raw_error) = apply_overview_nowcast_prediction(
        &state.pool,
        &mut payload,
        start_date.as_str(),
        end_date.as_str(),
