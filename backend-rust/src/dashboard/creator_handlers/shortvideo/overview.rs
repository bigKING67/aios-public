...
use super::super::super::{
    ...
    params::CreatorShortVideoOverviewQueryParams,
    query_runtime::run_dashboard_query_json,
    responses::{json_message_response, json_value_response},
    validation::{
        can_access_creator_shortvideo_dashboard, get_validated_date_param,
        is_date_range_within_limit, is_start_not_after_end,
        resolve_creator_shortvideo_max_query_date_range_days,
    },
};

pub(crate) async fn get_creator_shortvideo_overview(
    State(state): State<Arc<AppState>>,
    Query(query): Query<CreatorShortVideoOverviewQueryParams>,
    ...
    let sql = build_creator_shortvideo_overview_query_sql(CreatorShortVideoOverviewSqlOptions {
    ...
    let payload = match run_dashboard_query_json(&state.pool, sql.as_str()).await {
        ...
        Err(raw_error) => {
            ...
            error!(
                target: "dashboard-creator-shortvideo-overview",
                raw_error = %raw_error,
                "query failed"
            );
