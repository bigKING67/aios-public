...
use super::super::super::{
    ...
    creator_live::{build_creator_live_details_query_sql, CreatorLiveDetailsSqlOptions},
    errors::normalize_creator_live_error_message,
    params::CreatorLiveDetailsQueryParams,
...
pub(crate) async fn get_creator_live_details(
    State(state): State<Arc<AppState>>,
    Query(query): Query<CreatorLiveDetailsQueryParams>,
    ...
    let sql = build_creator_live_details_query_sql(CreatorLiveDetailsSqlOptions {
    ...
    let payload = match run_dashboard_query_json(&state.pool, sql.as_str()).await {
        ...
        Err(raw_error) => {
            ...
            error!(target: "dashboard-creator-live-details", raw_error = %raw_error, "query failed");
