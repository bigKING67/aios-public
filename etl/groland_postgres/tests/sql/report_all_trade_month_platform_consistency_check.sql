...
BEGIN
  ...
  IF to_regclass('ads.report_all_trade_month_platform') IS NULL THEN
    RAISE EXCEPTION 'view ads.report_all_trade_month_platform not found';
  END IF;
  ...
  WHERE table_schema = 'ads'
    AND table_name = 'report_all_trade_month_platform'
    AND column_name = 'refund_amount';

  IF v_ambiguous_columns > 0 THEN
    RAISE EXCEPTION 'ads.report_all_trade_month_platform contains ambiguous refund columns, count: %', v_ambiguous_columns;
  END IF;
  ...
  FROM (
    ...
    SELECT
      ...
      COALESCE(r.platform, e.platform) AS platform
    FROM ads.report_all_trade_month_platform r
  ...
  IF v_mismatch_rows > 0 THEN
    RAISE EXCEPTION 'report_all_trade_month_platform vs all_trade_overview consistency failed, rows: %', v_mismatch_rows;
  END IF;

  RAISE NOTICE 'report_all_trade_month_platform consistency checks passed';
END;
$$;
