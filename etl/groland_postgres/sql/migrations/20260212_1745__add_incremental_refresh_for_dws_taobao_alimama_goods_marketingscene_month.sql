...
CREATE TABLE IF NOT EXISTS etl.taobao_alimama_goods_marketingscene_month_refresh_state (
  ...
  CONSTRAINT chk_taobao_alimama_goods_marketingscene_month_refresh_state_id CHECK (id = 1)
);

COMMENT ON TABLE etl.taobao_alimama_goods_marketingscene_month_refresh_state IS 'DWS月汇总增量刷新水位状态表。';
COMMENT ON COLUMN etl.taobao_alimama_goods_marketingscene_month_refresh_state.id IS '固定单行主键（恒为1）。';
COMMENT ON COLUMN etl.taobao_alimama_goods_marketingscene_month_refresh_state.last_dwd_etl_time IS '最近一次已处理的DWD etl_time水位。';
COMMENT ON COLUMN etl.taobao_alimama_goods_marketingscene_month_refresh_state.last_refresh_at IS '最近一次刷新执行时间。';
COMMENT ON COLUMN etl.taobao_alimama_goods_marketingscene_month_refresh_state.last_refresh_start_date IS '最近一次刷新窗口起始日期。';
COMMENT ON COLUMN etl.taobao_alimama_goods_marketingscene_month_refresh_state.last_refresh_end_date IS '最近一次刷新窗口结束日期。';

INSERT INTO etl.taobao_alimama_goods_marketingscene_month_refresh_state (id)
VALUES (1)
ON CONFLICT (id) DO NOTHING;

DROP PROCEDURE IF EXISTS dws.refresh_taobao_alimama_goods_marketingscene_month_incremental(INTEGER, BOOLEAN);

CREATE PROCEDURE dws.refresh_taobao_alimama_goods_marketingscene_month_incremental(
...
BEGIN
  ...
  IF to_regclass('dws.taobao_alimama_goods_marketingscene_month') IS NULL THEN
    RAISE EXCEPTION 'target table dws.taobao_alimama_goods_marketingscene_month does not exist';
  END IF;
  ...
  IF p_init_watermark_only THEN
    ...
    UPDATE etl.taobao_alimama_goods_marketingscene_month_refresh_state
    SET
  ...
  CALL dws.refresh_taobao_alimama_goods_marketingscene_month(v_min_date, v_max_date);
...
COMMENT ON PROCEDURE dws.refresh_taobao_alimama_goods_marketingscene_month_incremental(INTEGER, BOOLEAN)
IS 'DWS月汇总增量刷新：依据DWD etl_time水位识别变更并刷新月汇总。';
