...
BEGIN
  ...
  INTO v_has_user_pay_amount;

  IF NOT v_has_user_pay_amount THEN
    ALTER TABLE ads.all_trade_overview
      ADD COLUMN user_pay_amount NUMERIC(18, 2);

    UPDATE ads.all_trade_overview
    SET user_pay_amount = COALESCE(gmv, 0);
  END IF;

  ALTER TABLE ads.all_trade_overview
    ALTER COLUMN user_pay_amount SET DEFAULT 0;

  UPDATE ads.all_trade_overview
  SET user_pay_amount = COALESCE(user_pay_amount, gmv, 0)
  WHERE user_pay_amount IS NULL;

  ALTER TABLE ads.all_trade_overview
    ALTER COLUMN user_pay_amount SET NOT NULL;
END;
$$;

COMMENT ON COLUMN ads.all_trade_overview.gmv IS '交易总额。抖音来源 ods.douyin_trade_sale_raw.trade_amount；未剔除退款。';
COMMENT ON COLUMN ads.all_trade_overview.user_pay_amount IS '用户支付金额。抖音来源 ods.douyin_trade_sale_raw.user_pay_amount；不含平台补贴、智能优惠券、达人补贴等补贴金额。';
COMMENT ON COLUMN ads.all_trade_overview.refund_amount_pay_time IS '当前成交退款金额（支付时间）。抖音来源 ods.douyin_trade_sale_raw.trade_refund_amount_pay_time。';
COMMENT ON COLUMN ads.all_trade_overview.refund_amount_refund_time IS '成交退款金额（退款时间）。抖音来源 ods.douyin_trade_sale_raw.trade_refund_amount_refund_time。';
...
BEGIN
  ...
  CREATE TEMP TABLE tmp_ads_all_trade_overview_platform_new (
    ...
    user_pay_amount NUMERIC(18, 2) NOT NULL,
    cost NUMERIC(18, 2),
