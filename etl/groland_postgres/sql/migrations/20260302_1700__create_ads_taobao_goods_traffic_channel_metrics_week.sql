...
DROP FUNCTION IF EXISTS ads.fn_touch_taobao_goods_traffic_channel_metrics_week_updated_at();
DROP TABLE IF EXISTS etl.taobao_goods_traffic_channel_metrics_week_refresh_state;
DROP TABLE IF EXISTS ads.taobao_goods_traffic_channel_metrics_week;

CREATE TABLE ads.taobao_goods_traffic_channel_metrics_week (
  ...
  pay_amount_delta NUMERIC(18, 2) NOT NULL DEFAULT 0,
  ...
  curr_cart_buyer_count BIGINT NOT NULL DEFAULT 0,
  prev_cart_buyer_count BIGINT NOT NULL DEFAULT 0,
  curr_pay_conversion_rate NUMERIC(10, 4),
  prev_pay_conversion_rate NUMERIC(10, 4),
  curr_cart_rate NUMERIC(10, 4),
  prev_cart_rate NUMERIC(10, 4),
  pay_amount_delta_contribution_rate NUMERIC(10, 4),
  created_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
  CONSTRAINT pk_taobao_goods_traffic_channel_metrics_week PRIMARY KEY (week_period, platform, product_id, traffic_channel),
  CONSTRAINT chk_taobao_goods_traffic_channel_metrics_week_platform CHECK (platform IN ('taobao')),
  CONSTRAINT chk_taobao_goods_traffic_channel_metrics_week_channel CHECK (
    traffic_channel IN ('搜索', '推荐', '关键词推广', '人群推广', '场景推广')
  ),
  CONSTRAINT chk_taobao_goods_traffic_channel_metrics_week_observed_days_range CHECK (
    observed_days IS NULL OR observed_days BETWEEN 1 AND 7
  )
);

COMMENT ON TABLE ads.taobao_goods_traffic_channel_metrics_week IS 'ADS-天猫商品流量渠道周归因指标表（周六至周五）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.week_period IS '周时间段，格式: 2025/2/7～2025/2/13。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.platform IS '平台标识，当前固定 taobao。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.product_id IS '商品ID。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.traffic_channel IS '流量渠道枚举（搜索/推荐/关键词推广/人群推广/场景推广）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.product_name IS '商品名称（优先本周窗口）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.as_of_date IS '同期口径截止日期（与 ads.all_trade_week_platform 对齐）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.observed_days IS '同期对比已观察天数（as_of_date - 周起始 + 1）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.curr_pay_amount IS '本周同期窗口渠道支付金额。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.prev_pay_amount IS '上周同期窗口渠道支付金额（向前平移7天）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.pay_amount_delta IS '渠道支付金额增量（curr_pay_amount - prev_pay_amount）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.curr_pay_buyer_count IS '本周同期窗口渠道支付人数。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.prev_pay_buyer_count IS '上周同期窗口渠道支付人数。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.curr_visitor_count IS '本周同期窗口渠道访客数。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.prev_visitor_count IS '上周同期窗口渠道访客数。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.curr_cart_buyer_count IS '本周同期窗口渠道加购人数。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.prev_cart_buyer_count IS '上周同期窗口渠道加购人数。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.curr_pay_conversion_rate IS '本周同期窗口渠道支付转化率（支付人数/访客）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.prev_pay_conversion_rate IS '上周同期窗口渠道支付转化率（支付人数/访客）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.curr_cart_rate IS '本周同期窗口渠道加购率（加购人数/访客）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.prev_cart_rate IS '上周同期窗口渠道加购率（加购人数/访客）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.pay_amount_delta_contribution_rate IS '渠道支付金额增量贡献率（pay_amount_delta / 周总增量）。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.created_at IS '记录创建时间。';
COMMENT ON COLUMN ads.taobao_goods_traffic_channel_metrics_week.updated_at IS '记录最后更新时间。';
COMMENT ON CONSTRAINT pk_taobao_goods_traffic_channel_metrics_week ON ads.taobao_goods_traffic_channel_metrics_week IS '主键：week_period + platform + product_id + traffic_channel。';
COMMENT ON CONSTRAINT chk_taobao_goods_traffic_channel_metrics_week_platform ON ads.taobao_goods_traffic_channel_metrics_week IS '平台枚举约束：当前仅 taobao。';
COMMENT ON CONSTRAINT chk_taobao_goods_traffic_channel_metrics_week_channel ON ads.taobao_goods_traffic_channel_metrics_week IS '流量渠道枚举约束。';
COMMENT ON CONSTRAINT chk_taobao_goods_traffic_channel_metrics_week_observed_days_range ON ads.taobao_goods_traffic_channel_metrics_week IS 'observed_days 范围约束：1~7（允许NULL）。';

CREATE INDEX idx_taobao_goods_traffic_channel_metrics_week_week_platform_delta
  ON ads.taobao_goods_traffic_channel_metrics_week (week_period, platform, pay_amount_delta DESC);
COMMENT ON INDEX ads.idx_taobao_goods_traffic_channel_metrics_week_week_platform_delta IS '按周按平台查看渠道增量Top的排序索引。';

CREATE INDEX idx_taobao_goods_traffic_channel_metrics_week_product
  ON ads.taobao_goods_traffic_channel_metrics_week (product_id);
COMMENT ON INDEX ads.idx_taobao_goods_traffic_channel_metrics_week_product IS '按商品ID检索加速索引。';

CREATE INDEX idx_taobao_goods_traffic_channel_metrics_week_channel
  ON ads.taobao_goods_traffic_channel_metrics_week (traffic_channel);
COMMENT ON INDEX ads.idx_taobao_goods_traffic_channel_metrics_week_channel IS '按流量渠道聚合分析索引。';

CREATE FUNCTION ads.fn_touch_taobao_goods_traffic_channel_metrics_week_updated_at()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
COMMENT ON FUNCTION ads.fn_touch_taobao_goods_traffic_channel_metrics_week_updated_at() IS '更新前自动刷新 taobao_goods_traffic_channel_metrics_week.updated_at 字段。';

CREATE TRIGGER trg_touch_taobao_goods_traffic_channel_metrics_week_updated_at
BEFORE UPDATE ON ads.taobao_goods_traffic_channel_metrics_week
FOR EACH ROW
EXECUTE FUNCTION ads.fn_touch_taobao_goods_traffic_channel_metrics_week_updated_at();
COMMENT ON TRIGGER trg_touch_taobao_goods_traffic_channel_metrics_week_updated_at ON ads.taobao_goods_traffic_channel_metrics_week IS '更新行时自动刷新 updated_at。';
...
CREATE TABLE etl.taobao_goods_traffic_channel_metrics_week_refresh_state (
  ...
  created_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
  CONSTRAINT chk_taobao_goods_traffic_channel_metrics_week_refresh_state_id CHECK (id = 1)
);

COMMENT ON TABLE etl.taobao_goods_traffic_channel_metrics_week_refresh_state IS 'ADS 天猫商品流量渠道周归因指标增量刷新水位状态表。';
COMMENT ON COLUMN etl.taobao_goods_traffic_channel_metrics_week_refresh_state.id IS '固定单行主键（恒为1）。';
COMMENT ON COLUMN etl.taobao_goods_traffic_channel_metrics_week_refresh_state.last_source_updated_at IS '最近一次已处理的上游更新时间水位。';
COMMENT ON COLUMN etl.taobao_goods_traffic_channel_metrics_week_refresh_state.last_refresh_at IS '最近一次 ADS 刷新执行时间。';
COMMENT ON COLUMN etl.taobao_goods_traffic_channel_metrics_week_refresh_state.last_refresh_start_date IS '最近一次 ADS 刷新窗口起始日期。';
COMMENT ON COLUMN etl.taobao_goods_traffic_channel_metrics_week_refresh_state.last_refresh_end_date IS '最近一次 ADS 刷新窗口结束日期。';
COMMENT ON COLUMN etl.taobao_goods_traffic_channel_metrics_week_refresh_state.created_at IS '记录创建时间。';
COMMENT ON COLUMN etl.taobao_goods_traffic_channel_metrics_week_refresh_state.updated_at IS '记录更新时间。';
COMMENT ON CONSTRAINT chk_taobao_goods_traffic_channel_metrics_week_refresh_state_id ON etl.taobao_goods_traffic_channel_metrics_week_refresh_state IS '固定单行约束（id=1）。';

INSERT INTO etl.taobao_goods_traffic_channel_metrics_week_refresh_state (id)
VALUES (1)
ON CONFLICT (id) DO NOTHING;

CREATE PROCEDURE ads.refresh_taobao_goods_traffic_channel_metrics_week_incremental(
...
BEGIN
  ...
  FROM etl.taobao_goods_traffic_channel_metrics_week_refresh_state
  WHERE id = 1
  FOR UPDATE;
  ...
  IF p_init_watermark_only THEN
    UPDATE etl.taobao_goods_traffic_channel_metrics_week_refresh_state
    SET
  ...
  SELECT MAX(stat_date) INTO v_fallback_end_date FROM dwd.taobao_goods_sale_traffic;
  ...
  CALL ads.refresh_taobao_goods_traffic_channel_metrics_week(v_refresh_start_date, v_refresh_end_date);
...
COMMENT ON PROCEDURE ads.refresh_taobao_goods_traffic_channel_metrics_week_incremental(INTEGER, BOOLEAN)
IS '按增量水位刷新 ADS 天猫商品流量渠道周归因指标，支持仅初始化水位。';
