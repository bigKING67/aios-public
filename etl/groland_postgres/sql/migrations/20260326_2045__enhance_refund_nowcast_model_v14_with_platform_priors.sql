...
DECLARE
  ...
  v_model_version TEXT := format('v1.4_platform_prior_bound_%sd', p_maturity_days);
...
BEGIN
  ...
  WITH platform_defaults AS (
    ...
    FROM (
      VALUES
        ('douyin', 0.45::NUMERIC, 0.58::NUMERIC, 0.42::NUMERIC, 0.95::NUMERIC),
        ('jd', 0.40::NUMERIC, 0.55::NUMERIC, 0.40::NUMERIC, 1.00::NUMERIC),
        ('taobao', 0.55::NUMERIC, 0.62::NUMERIC, 0.48::NUMERIC, 0.85::NUMERIC),
        ('wx', 0.45::NUMERIC, 0.57::NUMERIC, 0.42::NUMERIC, 0.90::NUMERIC),
        ('xhs', 0.50::NUMERIC, 0.60::NUMERIC, 0.45::NUMERIC, 0.85::NUMERIC)
    ) AS t(platform, cap_fallback_rate, prior_start_ratio, prior_floor_ratio, prior_power)
  ),
  finalized_hist AS (
    SELECT
      t.platform,
          ...
          LEAST(
            ...
            GREATEST(
              ...
              (
                ...
                ) * (1.0::NUMERIC - LEAST(1.0::NUMERIC, COALESCE(p.curve_sample_count, 0)::NUMERIC / 40.0::NUMERIC))
                + COALESCE(
                  ...
                  ) * LEAST(1.0::NUMERIC, COALESCE(p.curve_sample_count, 0)::NUMERIC / 40.0::NUMERIC)
              )
            )
          )
  ...
  pred AS (
    SELECT
      r.*,
      CASE
        WHEN r.refund_amount_pay_time_current <= 0 AND r.refund_amount_refund_time <= 0 THEN 0::NUMERIC(18, 2)
        WHEN r.age_days >= p_maturity_days THEN GREATEST(r.refund_amount_pay_time_current, r.refund_amount_refund_time)::NUMERIC(18, 2)
        ELSE ROUND(
          GREATEST(
            r.refund_amount_pay_time_current,
            r.refund_amount_refund_time,
            (
              r.refund_amount_pay_time_current
  ...
  bounded AS (
    SELECT
      ...
      CASE
        WHEN p.refund_amount_pay_time_current <= 0 AND p.refund_amount_refund_time <= 0 THEN 0::NUMERIC(18, 2)
        ELSE ROUND(
