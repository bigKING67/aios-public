...
-- dwd.all_trade_sale.refund_amount 是历史单字段退款口径。
-- 为兼容既有 DWS/周月汇总语义，这里保持“退款时间”口径；
-- 经营看板的“支付时间”GSV/退款率继续使用 ads.all_trade_overview 的双字段口径。
CREATE OR REPLACE PROCEDURE dwd.refresh_all_trade_sale_platform(
...
COMMENT ON COLUMN dwd.all_trade_sale.refund_amount
IS '退款金额（退款时间口径），来源为 ODS refund_amount_refund_time。';

COMMENT ON COLUMN dws.all_trade_sale_daily.refund_amount
IS '全渠道当日退款金额汇总（退款时间口径），来源为 dwd.all_trade_sale.refund_amount。';
