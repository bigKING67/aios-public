...
WITH normalized AS (
  SELECT
    ...
    ARRAY(
      ...
      FROM (
        SELECT DISTINCT ads.fn_influencer_library_normalize_anchor_tag(raw_tag) AS normalized_tag
        FROM (
...
SET
  ...
  anchor_desc = CASE
    WHEN cardinality(normalized.normalized_tags) > 0 THEN array_to_string(normalized.normalized_tags, '、')
    ELSE library.anchor_desc
  END,
  updated_at = NOW()
FROM normalized
WHERE library.id = normalized.id
  AND (
    library.tags IS DISTINCT FROM normalized.normalized_tags
    OR (
      cardinality(normalized.normalized_tags) > 0
      AND library.anchor_desc IS DISTINCT FROM array_to_string(normalized.normalized_tags, '、')
    )
  );
