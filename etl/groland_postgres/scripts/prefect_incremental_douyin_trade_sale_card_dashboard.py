...
REFRESH_RESULT_PATTERN = re.compile(
  r"refresh_douyin_trade_sale_card(?:_detail)? completed,\s*inserted_rows:\s*(\d+),\s*deleted_rows:\s*(\d+),\s*window:\s*\[([^\]]+)\]",
  re.IGNORECASE,
)
...
@task(name="init-douyin-trade-sale-card-dashboard-watermark", retries=1, retry_delay_seconds=30)
def init_watermark(fallback_window_days: int) -> str:
  ...
  output = run_psql(
    f"CALL ads.refresh_douyin_trade_sale_card_dashboard_incremental({fallback_window_days}, TRUE);"
  )
  if output:
    logger.info(output)
  return output
...
@task(name="refresh-douyin-trade-sale-card-dashboard-incremental", retries=2, retry_delay_seconds=120)
def run_incremental_refresh(fallback_window_days: int) -> str:
  ...
  output = run_psql(
    f"CALL ads.refresh_douyin_trade_sale_card_dashboard_incremental({fallback_window_days}, FALSE);"
  )
  if output:
    logger.info(output)
  return output
...
@task(name="build-douyin-trade-sale-card-dashboard-incremental-summary")
def build_incremental_summary(raw_output: str) -> Tuple[List[str], bool]:
  ...
  for line in notice_lines:
    ...
    if refresh_matched:
      ...
      table_label = "商品卡流量来源" if "card_detail" in line.lower() else "商品卡主表"
      total_inserted += inserted_rows
      total_deleted += deleted_rows
      details.append(f"{table_label}刷新窗口：{window}，插入 {inserted_rows}，删除 {deleted_rows}")
      continue
...
@flow(name="incremental-refresh-ads-douyin-trade-sale-card-dashboard-flow")
def incremental_refresh_douyin_trade_sale_card_dashboard_flow(
  ...
  if init_watermark_only:
    ...
    _safe_send_notification(
      title="ADS 抖音商品卡看板增量水位初始化通知",
      table_name="etl.douyin_trade_sale_card_dashboard_refresh_state",
      action=action,
      status=status,
      reason=reason,
  ...
  try:
    ...
    reason = str(error)
    _safe_send_notification(
      title="ADS 抖音商品卡看板增量刷新通知",
      table_name="ads.douyin_trade_sale_card / ads.douyin_trade_sale_card_detail",
      action=action,
      status=status,
      reason=reason,
      detail_lines=detail_lines,
...
if __name__ == "__main__":
  incremental_refresh_douyin_trade_sale_card_dashboard_flow()
