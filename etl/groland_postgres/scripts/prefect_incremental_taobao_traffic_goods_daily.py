...
REFRESH_RESULT_PATTERN = re.compile(
  r"refresh_taobao_traffic_goods_daily completed,\s*inserted:\s*(\d+),\s*deleted:\s*(\d+),\s*window:\s*\[([^\]]+)\]",
  re.IGNORECASE,
)
...
@task(name="init-taobao-traffic-goods-daily-watermark", retries=1, retry_delay_seconds=30)
def init_watermark(fallback_window_days: int) -> str:
  ...
  output = run_psql(
    f"CALL ads.refresh_taobao_traffic_goods_daily_incremental({fallback_window_days}, TRUE);"
  )
  if output:
    logger.info(output)
  return output
...
@task(name="refresh-taobao-traffic-goods-daily-incremental", retries=2, retry_delay_seconds=120)
def run_incremental_refresh(fallback_window_days: int) -> str:
  ...
  output = run_psql(
    f"CALL ads.refresh_taobao_traffic_goods_daily_incremental({fallback_window_days}, FALSE);"
  )
...
@task(name="build-taobao-traffic-goods-daily-incremental-summary")
def build_incremental_summary(raw_output: str) -> Tuple[List[str], bool]:
  ...
  refresh_lines = [
    ...
    if "refresh_taobao_traffic_goods_daily completed" in line.lower()
  ]
  skipped_lines = [
    line for line in notice_lines
...
@flow(name="incremental-refresh-ads-taobao-traffic-goods-daily-flow")
def incremental_refresh_taobao_traffic_goods_daily_flow(
  ...
  if init_watermark_only:
    ...
    _safe_send_notification(
      title="ADS 天猫商品流量日粒度增量水位初始化通知",
      table_name="etl.taobao_traffic_goods_daily_refresh_state",
      action=action,
      status=status,
      reason=reason,
  ...
  try:
    ...
    reason = str(error)
    _safe_send_notification(
      title="ADS 天猫商品流量日粒度增量刷新通知",
      table_name="ads.taobao_traffic_goods_daily",
      action=action,
      status=status,
      reason=reason,
      detail_lines=detail_lines,
...
if __name__ == "__main__":
  incremental_refresh_taobao_traffic_goods_daily_flow()
