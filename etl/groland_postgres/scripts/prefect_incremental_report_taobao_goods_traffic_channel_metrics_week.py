...
REFRESH_RESULT_PATTERN = re.compile(
  r"refresh_report_taobao_goods_traffic_channel_metrics_week completed,\s*inserted:\s*(\d+),\s*deleted:\s*(\d+),\s*window:\s*\[([^\]]+)\]",
  re.IGNORECASE,
)
...
@task(name="init-taobao-goods-traffic-channel-metrics-week-watermark", retries=1, retry_delay_seconds=30)
def init_watermark(fallback_window_days: int) -> str:
  ...
  output = run_psql(
    f"CALL ads.refresh_report_taobao_goods_traffic_channel_metrics_week_incremental({fallback_window_days}, TRUE);"
  )
  if output:
    logger.info(output)
  return output
...
@task(name="refresh-taobao-goods-traffic-channel-metrics-week-incremental", retries=2, retry_delay_seconds=120)
def run_incremental_refresh(fallback_window_days: int) -> str:
  ...
  output = run_psql(
    f"CALL ads.refresh_report_taobao_goods_traffic_channel_metrics_week_incremental({fallback_window_days}, FALSE);"
  )
...
@task(name="build-taobao-goods-traffic-channel-metrics-week-incremental-summary")
def build_incremental_summary(raw_output: str) -> Tuple[List[str], bool]:
  ...
  refreshed_raw_lines = [
    ...
    if "refresh_report_taobao_goods_traffic_channel_metrics_week completed" in line.lower()
  ]
...
@flow(name="incremental-refresh-ads-report-taobao-goods-traffic-channel-metrics-week-flow")
def incremental_refresh_report_taobao_goods_traffic_channel_metrics_week_flow(
  ...
  if init_watermark_only:
    ...
    _safe_send_notification(
      title="ADS 天猫商品流量渠道周归因增量水位初始化通知",
      table_name="etl.report_taobao_goods_traffic_channel_metrics_week_refresh_state",
      action=action,
      status=status,
      reason=reason,
  ...
  try:
    ...
    reason = str(error)
    _safe_send_notification(
      title="ADS 天猫商品流量渠道周归因增量刷新通知",
      table_name="ads.report_taobao_goods_traffic_channel_metrics_week",
      action=action,
      status=status,
      reason=reason,
      detail_lines=detail_lines,
...
if __name__ == "__main__":
  incremental_refresh_report_taobao_goods_traffic_channel_metrics_week_flow()
