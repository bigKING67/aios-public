...
REFRESH_RESULT_PATTERN = re.compile(
  r"refresh_douyin_live_goods_detail completed,\s*inserted_rows:\s*(\d+),\s*deleted_rows:\s*(\d+),\s*window:\s*\[([^\]]+)\]",
  re.IGNORECASE,
)
...
@task(name="init-douyin-live-goods-detail-watermark", retries=1, retry_delay_seconds=30)
def init_watermark(fallback_window_days: int) -> str:
  ...
  output = run_psql(
    f"CALL ads.refresh_douyin_live_goods_detail_incremental({fallback_window_days}, TRUE);"
  )
  if output:
    logger.info(output)
  return output
...
@task(name="refresh-douyin-live-goods-detail-incremental", retries=2, retry_delay_seconds=120)
def run_incremental_refresh(fallback_window_days: int) -> str:
  ...
  output = run_psql(
    f"CALL ads.refresh_douyin_live_goods_detail_incremental({fallback_window_days}, FALSE);"
  )
  if output:
    logger.info(output)
  return output
...
@task(name="build-douyin-live-goods-detail-incremental-summary")
...
@flow(name="incremental-refresh-ads-douyin-live-goods-detail-flow")
def incremental_refresh_douyin_live_goods_detail_flow(
  ...
  if init_watermark_only:
    ...
    _safe_send_notification(
      title="ADS 抖音直播商品明细增量水位初始化通知",
      table_name="etl.douyin_live_goods_detail_refresh_state",
      action=action,
      status=status,
      reason=reason,
  ...
  try:
    ...
    reason = str(error)
    _safe_send_notification(
      title="ADS 抖音直播商品明细增量刷新通知",
      table_name="ads.douyin_live_goods_detail",
      action=action,
      status=status,
      reason=reason,
      detail_lines=detail_lines,
...
if __name__ == "__main__":
  incremental_refresh_douyin_live_goods_detail_flow()
